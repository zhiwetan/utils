...
工作时候很多使用别人的脚本遇到报错，脚本没有编辑权限，找脚本文件写入权限所有者调试很麻烦。本文分享一种使用容器技术的方法，达到给别人脚本加打印的效果。
...
使用-r参数还有一个原因：内核会检查执行mount的程序uid是否为0，不为0会有权限问题，如果为0即使是使用上述方法map而来仍可允许。
...
如果对unshare命令实现有兴趣可以参考本项目ns/unshare.py，python版本至少需要3.12。

### 具体操作

unshare -m -U -r --propagation slave csh 进入新的命令行。所有相关操作在这个csh下执行才生效。
...
由于所有系统调用都是内核预期的使用方式。本文的方式不会造成操作系统层面的安全风险，或对同系统下其他用户产生影响，其他用户读取脚本时还是原来的文件。只在unshare后面的csh及其后代进程中生效。
...
还有一些脚本和程序根据uid推断用户名进一步推断home目录也无法工作，会出现尝试对/root目录的访问失败。有三种办法：一是可以在脚本报错附近修改home目录（使用bind方法修改脚本），无法修改的程序（如二进制文件）不适用。二是mount --bind ~ /root，本方法可能存在污染home目录的风险，尤其是如果某些程序存在把uid和username写入home目录的行为（通常不会）。三是mount -t tmpfs tmpfs /root，本方法/root为空的，可能一些程序需要的文件需要额外创建。
